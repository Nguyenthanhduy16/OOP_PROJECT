...
public class UserHandle {
 ...
 // Tìm JsonNode student theo userID
    private static JsonNode findStudentNodeByUserID(String userID) {
        List<JsonNode> rawUsers = loadRawUsers();
        for (JsonNode node : rawUsers) {
            if (node.has("userID") && node.get("userID").asText().equals(userID) && node.has("registeredActivities")) {
                return node;
            }
        }
        return null;
 ...
 // Lấy danh sách registeredActivities của student theo userID
 public static List<Activity> getRegisteredActivities(String userID) {
     JsonNode studentNode = findStudentNodeByUserID(userID);
	...
	//Cập nhật registeredActivities của student theo userID
	...
	// Thêm activity cho student theo userID
	public static void addActivityToStudent(String userID, Activity newActivity) {
	    List<Activity> activities = getRegisteredActivities(userID);
	    activities.add(newActivity);
	    updateRegisteredActivities(userID, activities);
	}

	// Xóa activity khỏi registeredActivities student theo userID
