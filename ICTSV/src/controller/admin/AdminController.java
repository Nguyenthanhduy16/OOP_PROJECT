...
//        }
    ...
    public void initialize() {
        ...
        if (activityList.isEmpty()) {
            activityList.addAll(
                    new Activities("Chìa khoá học tốt", "Học tập", "C1", "10","08:00","2023.2"),
                    new Activities("Ngày hội thể thao", "Xã hội", "Sân vận động","15","09:00","2024.2")
            );
        ...
        ActCategorySelected.setItems(FXCollections.observableArrayList("Tất cả", "Học tập", "Xã hội", "Kỷ luật", "Ý thức"));
        ActivitiesCategorySelected.setItems(FXCollections.observableArrayList( "Học tập", "Xã hội", "Kỷ luật", "Ý thức"));
        ActCategorySelected.getSelectionModel().select("Tất cả");
        ActivitiesCategorySelected.setValue("Chọn loại");
        ActSemesterSelected.setItems(FXCollections.observableArrayList("2023.1", "2023.2", "2024.1", "2024.2"));
        ActSemesterSelected.setValue("Chọn kỳ");
    }

    // Hàm này để luôn đọc lại dữ liệu từ file json qua Admin
    private void reloadActivityList() {
        activityList = FXCollections.observableArrayList();
        for (model.Activity a : admin.getAllActivities()) {
        ...
        System.out.println("Số lượng hoạt động đọc được: " + activityList.size());
        ActivitiesTable.setItems(activityList);
    }
    ...
    private void handleSearchActivity(ActionEvent event) {
        ...
        for (Activities activity : activityList) {
            ...
            boolean matchesSemester = (selectedSemester == null || selectedSemester.equals("Chọn kỳ") || activity.getSemester().equals(selectedSemester));
    ...
    private void handleAddAct(ActionEvent event) {
        ...
        model.Activity realAct = new model.Activity(category, name, semester, true, Integer.parseInt(scoreText), time, place);
        new model.Admin().addActivity(realAct);
        reloadActivityList();
    ...
    private void handleRemoveAct(ActionEvent event) {
        ...
        for (Activities act : toRemove) {
            model.Activity realAct = new model.Activity(
                    act.getCategory(),
                    act.getName(),
                    act.getSemester(),
                    true,
            ...
            new model.Admin().removeOutdatedActivity(realAct);
        }
//        reloadActivityList();
